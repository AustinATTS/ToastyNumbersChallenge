...
class ResultActivity : AppCompatActivity() {
  ...
  private fun showPlayResults() {
    ...
    val accuracy: Float = 100 - ((questions_incorrect.toFloat() / questions_count.toFloat()) * 100)
    ...
    analyseResults(
      questions_count,
      (questions_count - questions_incorrect),
      questions_incorrect,
      time,
      round_number,
      0,
    ...
    if (user != null) {
      ff_firestore
        ...
        .addOnSuccessListener { document ->
          if (document.exists()) {
            val account_total_questions = document.getLong("account_total_questions")?.toInt() ?: 0
            ...
            val account_highest_streak = document.getLong("account_highest_streak")?.toInt() ?: 0
  ...
  private fun saveStats(stat: String, value: Int) {
    ...
    val user = fa_auth.currentUser
    if (user != null) {
      val statsMap = mapOf(stat to value)
      ff_firestore
        .collection("users")
        .document(user.uid)
        .collection("statistics")
