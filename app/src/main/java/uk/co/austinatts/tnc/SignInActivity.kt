    ...
    auth.signInWithEmailAndPassword(email, password).addOnCompleteListener { task ->
      if (task.isSuccessful) {
        ...
        if (currentUser != null && currentUser.isAnonymous) {
          firestore.collection("users").document(currentUser.uid).delete().addOnCompleteListener {
            deleteFTask ->
        ...
        Toast.makeText(this, "Failed to sign in: ${task.exception?.message}", Toast.LENGTH_SHORT)
          .show()
      }
    }
  }
