...
class CustomActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    questions = intent.getIntExtra("QUESTIONS", 10)
    timel = intent.getIntExtra("TIME", 5)
      ...
      object : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable?) {
          ...
          if (autoAnswerEnabled) {
            if (
              answer_user.isNotEmpty() && answer_user.length >= answer_correct.toString().length
            ) {
              delayJob =
  ...
  private fun startTimer(time: Int) {
    ...
    if (time != 0) {
      timer =
        object : CountDownTimer(timer_duration, 10) {
  ...
  private fun checkAnswer(answer_user: Int) {
    ...
    if (answer_user == answer_correct) {
      ...
      if (questions_correct >= questions) {
        startNextRound()
      } else {
        generateQuestions()
      }
